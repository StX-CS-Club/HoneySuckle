...
public class Collision {
    ...
    //Uses SAT theorum to detect if two boxes overlap
    public static boolean isBoxOverlap(Point2D.Double origin, Point2D.Double size, double angle, Point2D.Double point, Point2D.Double pointSize) {
        ...
        final Point2D.Double halfSize = new Point2D.Double(size.x / 2, size.y / 2);
        final Point2D.Double halfPointSize = new Point2D.Double(pointSize.x / 2, pointSize.y / 2);

        // Find the corners of the rotated box
        final Point2D.Double[] originBoxCorners = getRotatedBoxCorners(origin, halfSize, angle);

        // Find the corners of the non-rotated box
        final Point2D.Double[] pointBoxCorners = getBoxCorners(point, halfPointSize);

        // Check for overlap using SAT (Separating Axis Theorem)
        return checkSAT(originBoxCorners, pointBoxCorners);
    }

    //Translates box corners via rotation
    ...
    private static boolean checkSAT(Point2D.Double[] box1, Point2D.Double[] box2) {
        for (int i = 0; i < box1.length; i++) {
            ...
            final Point2D.Double edge = new Point2D.Double(
                        box1[(i + 1) % box1.length].x - box1[i].x,
                        box1[(i + 1) % box1.length].y - box1[i].y
            );

            // Get the axis perpendicular to the edge
            final Point2D.Double axis = new Point2D.Double(-edge.y, edge.x);

            // Project both boxes onto the axis
            final double[] projection1 = projectOntoAxis(box1, axis);
            final double[] projection2 = projectOntoAxis(box2, axis);
